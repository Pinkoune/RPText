...
- **Récolte** (`gathering.ts`) : 4 métiers (bûcheronnage, minage, pêche, cueillette), tables de butin par biome, cooldown 2 min/métier, **niveaux de métier** (XP, ressources rares verrouillées par palier, bonus de rendement).
...
- **Talents** (`talents.ts`) : 1 point/niveau, passifs par classe (crit, réduction, esquive, double frappe, régén, furie, dégâts plats) agrégés en `CombatMods` et appliqués par `simulateCombat` (partagé chasse + donjons).
...
- **Multijoueur** :
  ...
  - **Boss mondial** (RTDB `world/boss`) via transactions atomiques, contributions et
    butin partagé au prorata.
