...
export default function MarketCard() {
  ...
  const sellable = useMemo(
    () => (p ? Object.entries(p.inventory).filter(([id, q]) => item(id)! && q > 0) : []),
    [p?.inventory],
  );
  ...
  const selItem = sel && item(sel)! ? item(sel)! : null;
    ...
    <div className="space-y-3">
      ...
      <div className="rounded-xl bg-black/25 p-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <select
            value={sel}
            onChange={(e) => { setSel(e.target.value); const it = item(e.target.value)!; if (it) setPrice(it.value); }}
            className="min-w-0 flex-1 rounded-lg bg-black/40 px-2 py-1.5 text-sm outline-none"
          >
            <option value="">— objet —</option>
            {sellable.map(([id, q]) => (
              <option key={id} value={id}>{item(id)!.name} ×{q}</option>
            ))}
          </select>
          <input
            type="number"
        ...
        <div>
          ...
          {myActive.map((l) => (
            <div key={l.id} className="flex items-center justify-between rounded-lg bg-black/25 px-3 py-2 text-sm">
              <span style={{ color: RARITY_COLOR[item(l.itemId)!.rarity] }}>{item(l.itemId)!.icon} {item(l.itemId)!.name}</span>
