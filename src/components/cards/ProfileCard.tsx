...
function Gear({ slot, id }: { slot: string; id: string | null }) {
  ...
  return (
    <div className="flex items-center justify-between rounded-lg bg-black/25 px-3 py-1.5 text-sm">
      ...
      <span style={{ color: it ? RARITY_COLOR[it.rarity] : '#64748b' }}>{it ? `${it.icon} ${it.name}` : '—'}</span>
    </div>
  );
}
        ...
        <div className="min-w-0 flex-1">
          {editing ? (
            <div className="space-y-1.5">
              ...
              <input value={title} onChange={(e) => setTitle(e.target.value.slice(0, 40))} placeholder="Titre / devise" className="w-full rounded-lg bg-black/40 px-2 py-1 text-xs outline-none focus:ring-1 focus:ring-sky-400/60" />
            ...
            <>
              ...
              {p.title && <div className="truncate text-xs italic text-amber-200/80">« {p.title} »</div>}
