...
import { computeAscensionBoss, ascensionOutcome, applyAscensionResult, neutralizeForNeant, ASCENSION_SUSTAIN_MULT, ASCENSION_SUSTAIN_ACTIVE_MULT, type AscensionBoss } from '../../game/ascension';
...
export default function AscensionCard() {
  ...
  function act(action: 'attack' | 'potion' | string, potionHeal?: number, potionId?: string) {
    ...
    const res = combatTurn(fightStats, mods, monster, fs.php, fs.bhp, action, { potionHeal: action === 'potion' ? potionHeal : 0, activeSkill: skill, sustainMult: ASCENSION_SUSTAIN_MULT, sustainActiveMult: ASCENSION_SUSTAIN_ACTIVE_MULT }, fs.combat);
